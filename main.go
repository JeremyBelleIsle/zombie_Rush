...
func input(mapX, mapY *float64, speed float64) {
...
func moveBullets(bullets []bullet) {
	for i := range bullets {
...
func bulletHitZombieReaction(zombieIndex int, bulletIndex int, zombies []zombie, bullets []bullet, upgrades map[string]int, lifes *int) ([]zombie, []bullet) {
...
func (g *Game) Update() error {
	if g.state == StatePlaying {
		...
		if len(g.cards) == 0 {
			...
			input(&g.mapX, &g.mapY, g.player.speed)
			...
			if bulletHitZombie {
				...
				g.zombies, g.bullets = bulletHitZombieReaction(zi, bi, g.zombies, g.bullets, g.upgrades, &g.player.lifes)
			}
